...
export class UserService {
  ...
  public sendEmailVerification(email: EmailModel): Observable<EmailModel | null> {
    const httpOptions = {
      headers : new HttpHeaders({'Content-Type': 'application/json'})
    }

    return this.http.post<EmailModel>('/api/user/send-email-verification', email, httpOptions).pipe(
      catchError(this.handleError<EmailModel>(`ERROR UserService.sendEmailVerification() : ${email.email}`))
    );
  }
  ...
  public forgotPassword(email: EmailModel): Observable<EmailModel | null> {
    const httpOptions = {
      headers : new HttpHeaders({ 'My-Custom-Header': 'foobar' })
    }

    return this.http.post<EmailModel>(`/api/user/forgot-password/`, email, httpOptions );
  }
  ...
  public resetPassword(password: string, id: string | null, token: string | null): Observable<PasswordModel | null> {
    ...
    return this.http.post<PasswordModel>(`/api/user/reset-password/${id}/${token}`, password, httpOptions );
  }
